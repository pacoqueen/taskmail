  ...
  /**
	...
	init : function() {
  	...
  	var nsIFolderListener = Components.interfaces.nsIFolderListener;
		...
		var folderListener = {
		  OnItemAdded: function(parent, item, viewString) {TASKMAIL.log("folderListener.OnItemAdded");},
		  OnItemRemoved: function(parent, item, viewString) {TASKMAIL.log("folderListener.OnItemRemoved");},
		  OnItemPropertyChanged: function(parent, item, viewString) {TASKMAIL.log("folderListener.3");},
		  OnItemIntPropertyChanged: function(aItem, aProperty, aOld, aNew) {var propertyString = aProperty.toString();TASKMAIL.log("folderListener.OnItemIntPropertyChanged,aProperty="+propertyString);},
	...
	showLinkedMail : function() {
		...
		if (selectedTask.length == 1) {
			...
			if (keysMails != null && keysMails.length > 0) {
				...
				TASKMAIL.log("nextTaskId={" + nextTaskId.folderURI + "," + nextTaskId.key + "}");
...
TASKMAIL.Link = {
	...
	dump : function () {
		...
		for(var i=0; i<this.links.length; i++) {
			result += "{" + this.links[i].folderURI + "," + this.links[i].key + "," + this.links[i].taskId + "},";
		}
		TASKMAIL.log("dump"+result);		
	},
	...
	keyChanged : function (aOldKey, aNewHdr) {
		TASKMAIL.log("keyChanged");
		var dumpavant = "";
		for(var i=0; i<this.links.length; i++) {
			dumpavant += this.links[i].key + ",";
		}
		TASKMAIL.log("keyChanged result=" + dumpavant);		
		for(var i=0; i<this.links.length; i++) {
			if (this.links[i].key == aOldKey) {
				this.links[i].key = aNewHdr.messageKey;
			}
		}
		var dumpapres = "";
		for(var i=0; i<this.links.length; i++) {
			dumpapres += this.links[i].key + ",";
		}
		TASKMAIL.log("keyChanged result=" + dumpapres);		
	},
...
TASKMAIL.MailListener = {
	...
	msgKeyChanged : function (aOldKey, aNewHdr){
		TASKMAIL.log("msgKeyChanged, aOldKey=" + aOldKey + ",aNewHdr.messageId=" + aNewHdr.messageId + ",aNewHdr.messageKey=" + aNewHdr.messageKey);
		TASKMAIL.Link.keyChanged(aOldKey, aNewHdr);
		var tree = document.getElementById("threadTree");
		var column = tree.columns.getNamedColumn("taskmail-colTask");
		tree.treeBoxObject.invalidateColumn(column);
