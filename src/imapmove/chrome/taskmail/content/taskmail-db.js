...
TASKMAIL.DB = {
	...
	getLinkSQLite : function(msgFolder, taskFolder, viewFilter) {
		TASKMAIL.log("getLinkSQLite,arguments:msgFolderName="+msgFolder.URI+"taskFolderName="+taskFolder.URI);
		try {
			...
			while (stat.executeStep()) {
				...
				TASKMAIL.log("getLinkSQLite, bd information, messageFolderURI=" + messageFolderURI + ",messageId=" + messageId);
				try {
					var folderDB = MailUtils.getFolderForURI(messageFolderURI, false); 
					TASKMAIL.log("getLinkSQLite,folderDB=" + folderDB.URI);
...
//          		Components.utils.reportError("getLinkSQLite, Purge réalisée");   
				...
				TASKMAIL.log("getLinkSQLite,link:" + messageFolderURI + ",messageId=" + messageId + ",messageKey=" + messageKey
					+ ",threadKey=" + threadKey + ",taskId=" + stat.getInt32(2));				
